 ...
 * 状态经 getProjectState() 读取（非 React 上下文不能调 useProject 这个 hook）。
 */
...
async function loadUseProject() {
  vi.resetModules();
  return import('@/hooks/useProject');
}
...
describe('fetchProjects 选中策略', () => {
  it('无本地记忆且后端无当前项目时，默认选中第一个项目并同步后端', async () => {
    ...
    const { fetchProjects, getProjectState } = await loadUseProject();
    await fetchProjects();

    expect(mockPost).toHaveBeenCalledWith('/api/projects/p1/switch', {});
    expect(getProjectState().currentProjectId).toBe('p1');
    expect(getProjectState().initialized).toBe(true);
  });
  ...
  it('本地记忆有效时沿用本地记忆，不落到第一个项目', async () => {
    ...
    expect(getProjectState().currentProjectId).toBe('p2');
  });
  ...
  it('项目列表为空时置空选中，initialized 置真供 Home 判定「暂无项目」', async () => {
    ...
    expect(getProjectState().initialized).toBe(false);
    await fetchProjects();

    expect(mockPost).not.toHaveBeenCalled();
    expect(getProjectState().currentProjectId).toBe('');
    expect(getProjectState().initialized).toBe(true);
  });
});
