 ...
 *
 * 注意：`useTokenUsage()` 是普通工厂函数、不是 React Hook（内部不调用任何 hook），
 * 这样轮询纪律能在 node 环境直接单测。组件请用下面的 `useTokenUsageState()`，
 * 它在工厂之上接了订阅，events/loading 变化时会触发重渲染。
 */
...
export interface TokenUsageState {
  events: Record<string, TokenUsageEvent[]>
  loading: Record<string, boolean>
}

export interface TokenUsageController {
  getState: () => TokenUsageState
  subscribe: (listener: () => void) => () => void
  fetchDetail: (turnId: string) => Promise<TurnUsageSummary | null>
  startPolling: (turnId: string, onUpdate: (summary: TurnUsageSummary) => void) => void
  stopPolling: (turnId: string) => void
  stopAll: () => void
}

export function useTokenUsage(): TokenUsageController {
  const store = createStore<TokenUsageState>({ events: {}, loading: {} })
  /** turnId → timer，保证同一 turn 只有一个 poller */
  const pollers = new Map<string, ReturnType<typeof setInterval>>()

  async function fetchDetail(turnId: string): Promise<TurnUsageSummary | null> {
    store.set((prev) => ({ ...prev, loading: { ...prev.loading, [turnId]: true } }))
    try {
      const { data } = await http.get(`/api/usage/turns/${turnId}`)
      store.set((prev) => ({ ...prev, events: { ...prev.events, [turnId]: data.events ?? [] } }))
      return data.summary as TurnUsageSummary
    } catch {
      // 404 = degraded 追踪未持久化，或会话已删；保留现有数据不动
      return null
    } finally {
      store.set((prev) => ({ ...prev, loading: { ...prev.loading, [turnId]: false } }))
    }
  }
  ...
  return { getState: store.get, subscribe: store.subscribe, fetchDetail, startPolling, stopPolling, stopAll }
}

/** 组件侧入口：同一个控制器活到组件卸载，卸载时自动停掉全部轮询。 */
export function useTokenUsageState(): TokenUsageController & TokenUsageState {
  const controller = useMemo(() => useTokenUsage(), [])
  ...
  useEffect(() => () => controller.stopAll(), [controller])

  return { ...controller, events: state.events, loading: state.loading }
}
