...
- ~~未与 DeepSeek 控制台做金额对账~~：已完成（2026-07-30，§9.6 两层对账，脚本
  `test/eval/_reconcile_balance.ts`）——①目录价与官方价格页逐项一致（v4-flash 0.02/1/2、
  v4-pro 0.025/3/6 元/M）；②隔离窗口 12 次独特长 prompt（210,588 未命中 + 12 输出 token），
  本地记账 ¥0.210612 vs 余额变化 ¥0.21（65.84→65.63），偏差 +0.3%，在余额 ¥0.01 粒度内。
  官方文档**不公布**的 alias 映射（deepseek-chat→v4-flash）由此经真实扣费证实
  （若实际按 v4-pro 计费，余额应掉 ¥0.63）。注：计费入账延迟约 3~5 分钟，对账脚本的
  余额稳定等待要按此设置；月度复查按 §9.6 重跑同款脚本即可。
...
- ~~eval 记账的活体对账未跑~~：已完成（2026-07-30，3 用例小轮）——5 个 `pipeline='eval'`
  turn 落库，`parent_turn_id` 指向对应 ask turn，9 张 `eval.judge` event 带真实 judge 模型；
  缓存命中的冲突 fixture 0 event（§11.3 语义）。runner 接入见评审 P1（394fa96）。
- ~~`REFLECT_JUDGE=1` 的反思裁决漏记账~~：已修（评审 P2，455bc56），`ask.reflection` /
  `agent.reflection` 两个 stage 激活，回归见 test/reflectionUsage.test.ts。
- 完整问题清单与修复要点见
  [交付评审问题清单](./2026-07-29-token-cost-tracking-review-findings.md)
  （P1–P9，其中 P1–P3 已修复，提交号见清单内各项标注）。
