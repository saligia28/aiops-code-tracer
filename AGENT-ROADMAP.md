...
是"机制已通、还差接线或数字"。每条开口都在下方 [可验收任务清单](#可验收任务清单t1t21) 里有编号任务。
...
| 项 | 主体已落地 | 开口 |
|---|---|---|
| **P0-A** 反思进循环 | ask 主路径 L1 引用核对 + L2 judge + 重答；**agent 管线（含证据收集层）已接**（T1 ✅ 2026-07-27） | T2（简单路径不接反思）、T3（流式不重试）、T4（重答后不二次核对） |
...
> 要看"现在该做什么"直接跳 [可验收任务清单](#可验收任务清单t1t21)。
...
>
> **副作用（2026-07-29 由 T19 的测量挖出，已由 T20 修复）**：agent 侧的重答会让模型**把行号全删掉**。
> 活体实证：`citationAccuracy=0.5` 触发重答 → 新答案开场"我之前的回答中编造了不存在的行号" →
> 4633 字里 `file:line` 引用 0 条。根因是 L1 反馈那句"无法确认的位置不要编造行号，改为在证据不足里说明"
> ——对 ask 管线成立（codeContext 在眼前，能改对），对 agent 不成立（**重答禁用工具** + 早期 read_file
> 结果已被 `compressMessages` 折叠，模型没能力改对，全删是唯一合规动作）。**即 T1 在 agent 上按"引用准确率"
> 优化，代价是引用密度归零**——指标变好、可追溯性变差的典型。
> **T20 已修**（保留引用 + 降级行号），并在修的过程中发现 T1 还漏抄了 `forceFinalAnswer` 里
> "不要再调用任何工具"那句——缺它时重答有 5/6 次吐的是工具调用标记而非答案。详见 T20。
...
>   需 K 次/任务 重复跑。
> - **加固（同日）· K 次/任务重复跑压方差**：`run.ts` 加 `EVAL_RUNS`（每任务 K 次，聚合均值+区间）+
>   `EVAL_TASK`（id 过滤，小成本验证/单条复跑）。K=3 一上就暴露**单条覆盖率方差极大**：preparation-order /
>   aftersale 区间 **0~5**、supplier-deduct **0~5**——单次跑分根本不可信（印证前面 supplier-deduct 单次
>   0/5 是方差、非坏样本）。**对照（ON 全 6 条 K=3 = 覆盖率 90%（71/79）；OFF 为控 token 中止在 4/6）**：
>   在 OFF 已完成的 4 条上 ON/OFF 覆盖**大致持平**（ON 15.3/18 vs OFF 16.3/18，OFF 略高）、工具相近——
>   **单次那版"ON 82% vs OFF 64%"的差主要是噪声、不是 planner 效应**（preparation-order 最典型：ON 一跑掉 0、
>   OFF 三跑全 5，纯方差翻转）。**结论回到 P1-C 原始直觉：方差主导、planner 对完整度无显著收益**——但这次
>   是在修好的指标 + 量化方差的工具上得出，比 P1-C 扎实。要下硬结论需 K 更大 + OFF 补全（本轮为控 token 未做完）。
...
**T1 · agent 管线接反思（含证据收集层）**〔P0-A · ~~1.5~2 天~~ · **已完成 2026-07-27**〕✅
- 交付：`agent/evidenceCollector.ts`（工具结果 → 行索引 → 按答案引用组装 `Evidence[]`）+ agentLoop
  收尾统一走 `finalizeAnswer`（自查在 `answer_delta` 之前，不合格发 `reflecting` 并重答 1 次）+
  路由记 `reflection` span。详见 P0-A 进度块。
- 验收结果：证据收集器 8 用例（喂真实工具输出，code 与源码逐字一致）+ 反思 5 用例（L1 真跑了 /
  恰好重答 1 次 / 重答失败放行 / 中止不触发 / 无引用诚实标注未跑）；**摘掉证据层即有 2 条转红**，
  证明不是同义反复。活体：22 次工具调用、7 条引用、citationAccuracy=1.0。
- **验收项③未做**：`eval -- agent` 完整覆盖率对照没跑——单次跑分方差区间 0~5（见 P1-C 加固记录），
  跑一次得不出"不回退"的结论；只做了单任务冒烟（t-price-check-flow 覆盖 4/4）。要下结论需 K≥3 全量重跑。
- 衍生任务见 T19。
...
**T19 · agent 的 evidence 随 done 下发（T1 衍生）**〔P0-A/评测 · **机制已完成 2026-07-29**，收益低于预期〕⚠️
- 交付：`done` 事件加 `evidence`（additive，重答后按**最终**答案重算——沿用旧的会下发指向已被更正掉的
  行号的假证据，测试已钉死）；`trace.end` 不再硬写 `evidence: []`；eval `askAgent` 透传给 judge；
  `EVAL_AGENT_NO_EVIDENCE=1` 保留旧口径供复现对照。
- **测量方法**（值得复用）：**同一份 agent 答案判两次**（judge 证据 OFF/ON），agent 只跑一次 →
  两组间**零 agent 方差**。直接跑两遍 `eval -- agent` 会把 agent 随机性混进来（覆盖率单次区间已知 0~5），
  那样得出的差值不可信。
- **结果（n=3 长任务，elink-pc + DeepSeek）**：忠实命中 OFF **0/3** → ON **0/3**；平均分 **4.0 → 5.3**；
  平均证据 **0.3 条/答案**。三条里只有 1 条答案带引用（1 条），那条分数 **3 → 7**；另外两条答案
  **一个 file:line 都没给**，judge 拿到的仍是空数组，口径没变、分数自然没变。
- **结论：机制通了，但根因不在这儿。** "judge 空手评 faithful" 的主因不是"证据没下发"，
  而是**长任务答案里压根没有可抽的行号引用**。原任务描述里"这句话的前提已经不成立了"说早了——
  前提只在"答案给了引用"时才不成立。run.ts 的读数提示已按实测改写（先看平均证据条数，再决定
  该怪答案还是怪口径）。
- **顺带挖出 T1 的副作用（本轮最有价值的发现，已登记为 T20）**：见下。

**T20 · agent 侧反思反馈要「修复引用」而不是「删掉引用」**〔P0-A · **已完成 2026-07-29**，部分达标〕⚠️

> **结果先行（配对实测，K=3 × 4 条件 = 12 次真实 LLM 调用，DeepSeek）**：
>
> | 条件 | 吐工具调用标记 | 引用条数 | 「未确认行号」降级 |
> |---|---|---|---|
> | 旧文案 · 上下文已折叠 | **2/3** ⚠️ | 0.7 | 0 |
> | 旧文案 · 上下文未折叠 | **3/3** ⚠️ | 0.0 | 0 |
> | 新文案 · 上下文已折叠 | **0/3** | 0.0 | **1.0** |
> | 新文案 · 上下文未折叠 | **0/3** | **2.3** | 0.7 |
>
> - **意外收获（比原定目标更值钱）**：旧文案下 **5/6 次重答吐的是 DSML 工具调用标记、不是答案**——
>   `tools: []` 只让 API 不返回结构化 tool_call，**system prompt 里的工具目录还在**，模型照样"想调工具"，
>   于是把调用意图当正文输出，被我们当最终答案原样下发。`forceFinalAnswer` 一直有"不要再调用任何工具"
>   这句，**反思重答没有**——T1 抄了半条。补上后 **0/6**。
> - **降级路径生效**：折叠上下文下 3/3 走「文件名（未确认行号）」，而不是静默删引用。
> - **引用能修好，前提是有据可查**：上下文未折叠时新文案平均产出 2.3 条引用（一次 7 条）；
>   折叠时 0 条——模型手里根本没有行号数据，这时"保住文件名 + 明示未确认"已是天花板。
> - **反例护栏通过**：12 次里"仍带错引用" **0/12**，降级没有退化成编造。
> - **未达标**：验收线「重答后 evidence 条数不低于重答前」只在**上下文未折叠**时成立；
>   折叠场景仍是 2 条 → 0 条。真正的解法是方案里那条更贵的路——**重答时放开只读工具**，已登记 T21。
> - **口径诚实标注**：合成上下文（单文件、单条错引用）、n=12，不是端到端；真实长上下文里模型更常
>   正常作答（T19 那次活体重答就是正常散文），所以"5/6 吐标记"是**最坏情况**下的数字，
>   不能读成"线上一直在吐垃圾"。修复本身是纯收益（0/6 vs 5/6），无需权衡。
> - 落地：`reflection.ts` 新增 `pipeline?: 'ask'|'agent'` 入参（缺省 ask，老调用方逐字不变）+
>   `buildCitationFeedback` 分化文案；`reflecting` 事件加 `evidenceCount`（重答**前**的引用数，
>   否则这条验收线永远量不出来）；测试 `reflectionFeedback.test.ts` 5 条钉住四要素
>   （先关工具意图 / 禁止删引用 / 有降级出口 / 禁止编造）+ agentLoop 断言重答用的是 agent 那套。

---

**T21 · 重答时放开只读工具（T20 未达标那半）**〔P0-A · 1 天 · 依赖 T20〕
- 现状：重答一律 `tools: []`。上下文被 `compressMessages` 折叠后，模型**没有能力**把行号改对，
  只能降级成「未确认行号」——T20 实测折叠场景引用条数 2 → 0。
- 做什么：重答时放开 `read_file` / `search_in_file` 两个只读工具（**只这两个**，不给搜索全仓的口子），
  轮次上限 1~2 轮，让模型回去把行号查准。
- **验收**：折叠上下文下重答后引用条数 **≥ 重答前**且 `citationAccuracy` 提升；
  代价侧记录多出的工具调用次数与耗时——**若单次重答成本翻倍而引用只多 1~2 条，就维持降级方案**
  （反思是增强不是闸门，不值得为它把 agent 成本抬上去）。
- **现象（活体实证，不是推测）**：一次长任务活体跑到 `citationAccuracy=0.5` → 触发重答 →
  新答案开场就是"您说得对，我之前的回答中编造了不存在的行号"，然后把**全部行号删光**：
  4633 字的答案里 `file:line` 引用 **0 条**、中文"第 N 行"**0 处**，只剩 10 个裸文件名。
  SSE 实据：`reflecting{citationAccuracy:0.5}` + `done{reflectionRetried:true, evidence:[]}`。
- **根因**：L1 反馈原文是"无法确认的位置不要编造行号，改为在「证据不足」里说明"
  （[reflection.ts](apps/api/src/services/ask/reflection.ts)）。这话对 **ask 管线**是对的——codeContext
  就在眼前，模型能照着改对。但 **agent 重答时禁用了工具**，且早期 read_file 结果已被
  `compressMessages` 折叠成面包屑——模型**没有能力**把行号改对，唯一合规动作就是全删。
- **代价**：① 可追溯性倒退（用户拿到一份没有任何行号的答案，比半对半错更难核）；
  ② 重答后的答案 evidence 恒空，L1 在它身上变成空转；③ T19 的收益被这条抵消掉大半。
- 做什么：反馈文案按管线分化。agent 侧改成"**保留文件引用，只修正或降级行号**"——能确认的给正确行号，
  不能确认的写 `文件名（未确认行号）`，**禁止整条删除引用**；或者放开重答时的只读工具（贵，先试文案）。
- **验收**：同一批长任务，重答后的 `evidence` 条数 **不低于**重答前（当前是"从 N 条掉到 0 条"）；
  `citationAccuracy` 提升而非靠清空引用规避；答案里裸文件名占比下降。
  **反例护栏**：若模型改为编造更多"看起来对"的行号（accuracy 反降），立即回滚——宁可少引用不可假引用。
...
1. **A 组**（~~T1~~ 已完成 → T2/T4/T5/T6 先做，T3 等前端交互，T7 等内网环境）
   ——共性是"某条路径有、另一条没有"的行为不一致（复杂路径注文档简单路径不注、API 模式流式内网
   模式不流式），代价多在半天到一天，且最容易在演示或面试里被一问就穿。
   **T1 印证了这条分组的成本假设是错的**：它看着是"接线"，实际因为 L1 的证据守卫而必须先补一层
   ——所以 A 组里剩下的几条也别默认都是半天活，动手前先确认接口能不能直接用。
2. **B 组**（~~T8~~ ~~T19~~ 已完成 → **T20 优先** → T9~T12）——把已落地的能力补上数字与活体佐证。
   两条教训都出自这一组：**T8**——"验收欠账"补起来往往不只是补测试，为了让注入用例真能红，
   顺手挖出并修掉了两个从未被防线覆盖的通道；**T19**——量化一个"顺手的小改进"时，
   真正的收获是发现 T1 让 agent 答案的引用密度归零（T20），而 T19 自己的收益反而低于预期。
   **T20 插到最前**：它是已上线功能的行为回退，比继续补数字优先。
