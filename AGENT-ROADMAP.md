...
是"机制已通、还差接线或数字"。每条开口都在下方 [可验收任务清单](#可验收任务清单t1t19) 里有编号任务。
...
> 要看"现在该做什么"直接跳 [可验收任务清单](#可验收任务清单t1t19)。
...
**T19 · agent 的 evidence 随 done 下发（T1 衍生）**〔P0-A/评测 · 0.5 天 · 依赖 T1（已完成）〕
- 现状：T1 之后 agent **已经有**结构化 `Evidence[]` 了，但只在 agentLoop 内部喂给反思，没往外发。
  `run.ts` 的 agent 评测因此仍给 judge 传 `evidence: []`，输出里那句"agent 答案不产出结构化 evidence，
  故 score 系统性偏低，仅供横向对照"说的就是这个——**现在这句话的前提已经不成立了**。
- 做什么：`done` 事件加 `evidence`（与 `citationAccuracy` 同款 additive 字段）；eval 的 `askAgent`
  透传给 `judgeAnswer`。
- **验收**：同一批长任务用例，judge 的 faithful 判定从"无引用可依"变为有据可判——记录接入前后的
  平均分与 faithful 命中，并据实更新 run.ts 那段读数提示（若分数没变，也照实写"口径变了但结论不变"）。
...
2. **B 组**（~~T8~~ 已完成 → T9~T12，新增 T19）——把已落地的能力补上数字与活体佐证。
